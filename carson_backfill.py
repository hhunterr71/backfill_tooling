    ...
    for idx, row in enumerate(mapping_rows, 1):
        ...
        try:
            ...
            with open(meta_path, 'w') as mf:
                json.dump({'type': meter_type or '', 'bug_number': bug_number}, mf)
...
def pivot_flat_file(input_path, outputdirname, meter_type=None, field_map=None, bug_number=''):
    """
...
def setup_job(parent_dir, job_name):
    ...
    with open(mapping_template_path, 'w', newline='') as f:
        ...
        writer.writerow(['building_id', 'meter_name', 'external_id', 'type', 'bug_number', 'start_date', 'end_date'])
        writer.writerow([
    ...
    for idx, filepath in enumerate(files, 1):
        ...
        try:
            pivot_flat_file(filepath, output_dir, meter_type=meter_type,
                            field_map=effective_field_map, bug_number=bug_number)
            successful += 1
            print(f"  Done")
        except Exception as e:
            failed += 1
